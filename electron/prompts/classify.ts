...
**关键信息提取规则**（判断3）：
- contract_amount：查找"合同总金额"、"合同金额"、"总价"、"合同总价"等关键词，提取纯数字（单位：元）。如"15.27万元" → 152700
- contract_items：提取合同分项明细，每项包含name(名称)、amount(金额数字)、description(描述)
...
**合同金额提取规则**（重要）：
- 查找"合同总金额"、"合同金额"、"总价"、"合同总价"等关键词
- 金额格式可能是"152,654.11元"、"15.27万元"、"¥152,654.11"等
- 提取纯数字（单位：元），如"15.27万元" → 152700
- 合同分项明细(contract_items)：提取每个分项的名称、金额和描述
