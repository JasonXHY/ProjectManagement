...
## v0.2.0 体验深化版本（2026-06-29 MiMoCode）

### 发版说明
全面升级项目详情页，新增卡片系统、利润测算、项目转交、Markdown预览等核心功能。426个测试全通过。

### ⭐ 新增功能

#### 项目卡片系统（10个卡片）
- **统计行**：文件数量 + 里程碑时间轴 + 待处理文件 + AI摘要入口
- **售前阶段**：项目信息 + 合同概览 + 项目评估（利润测算）
- **进行中阶段**：需求跟踪 + 关键问题 + 签字追踪
- **关闭阶段**：拓展商机 + 交付物清单 + 项目总结
- 每个卡片支持弹窗详情查看，数据从AI分析自动填充

#### ⭐ 利润测算工具
- 动态成员表格：支持添加/删除团队成员，按角色和职级自动计算单价
- 实时计算：输入即算，无需点击按钮
- 成本分配明细：内部/外包人天成本、差旅、总成本、合同分配
- 利润率三栏：内部/外包/整体利润率，带进度条和红线警告
- 数据持久化：测算结果自动保存，下次打开恢复上次数据
- 一键复制：复制为汇报格式或PPT大纲

#### ⭐ 项目转交功能
- 整体转交：将整个项目打包为.pmaer.zip文件
- 协作转交：AI智能选择相关文件进行转交
- 导入转交：导入.pmaer.zip文件，自动恢复项目结构、文件分类、AI分析结果
- 数据完整性：导入后保留文件分类、AI摘要、阶段信息，无需重复分析

#### ⭐ Markdown预览
- 支持项目文件中的Markdown内容渲染
- 代码高亮、HTML安全过滤
- AI对话中的Markdown回复自动渲染

#### AI分析增强
- 结构化提取：自动识别合同金额、客户信息、需求清单、关键问题
- 去重优化：需求/问题清单自动去重，排除验证类任务
- 合同金额校验：多文件提取时自动校验一致性
- 阶段推进：验收报告/项目总结/项目归档/复盘总结自动触发关闭阶段

### Bug修复
- MiMo AI超时：从60秒提升到120秒，解决摘要生成超时
- 元数据竞态：5处metadata写入添加全局队列，防止并发覆盖
- 触发子分类清理：移除不会生效的验收报告/项目总结，保留项目归档/复盘总结
